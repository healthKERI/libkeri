    ...
    pub fn build(self) -> Result<SerderKERI, Box<dyn Error>> {
        ...
        let isith = match self.isith {
            ...
            None => {
                ...
                let default_threshold = std::cmp::max(1, (self.keys.len() as f64 / 2.0).ceil() as usize);
        ...
        let nsith = match self.nsith {
            ...
            None => {
                ...
                let default_threshold = std::cmp::max(0, (self.ndigs.len() as f64 / 2.0).ceil() as usize);
                TholderSith::Integer(default_threshold)
            }
        };
        ...
        let kt = if self.intive && tholder.num().is_some() && tholder.num().unwrap() <= MAX_INT_THOLD {
        ...
        let nt = if self.intive && ntholder.num().is_some() && ntholder.num().unwrap() <= MAX_INT_THOLD {
...
mod tests {
    ...
    fn test_inception_event_builder_non_transferable() -> Result<(), Box<dyn Error>> {
        ...
        let serder = InceptionEventBuilder::new(keys0)
            .build()?;
